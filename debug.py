...
def get_candidates1(candiate_file,candiate_top):
    """
...
def get_candidates2(candiate_file,candiate_top):
    """
    get top ranked entity candidates for
    an event    
    """
...
def dict_check(original, candidate_model):
    for w in original:
        if w not in candidate_model:
            continue
        else:
            for word in original[w].model:
                if original[w].model[word] != candidate_model.model[w]:
...
def main():
    ...
    entity_candidates1 = get_candidates1(args.candiate_file,args.candiate_top)
    entity_candidates2 = get_candidates2(args.candiate_file,args.candiate_top)

    candidate_models1 = get_candidate_models(entity_candidates1,args.article_dir)
