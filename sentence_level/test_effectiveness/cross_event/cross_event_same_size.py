...
while keeping the size of the training data the same
"""
...
def main():
    ...
    all_data = load_data_set(args.feature_data_dir,args.split_data)

    feature_data = []
    for other_instance in all_data:
       feature_data +=  all_data[other_instance]
    ...
    inter_feature_data = random.sample(feature_data, len(instance_train_index))
    ...
    inter_top_context_feature, inter_top_category_feature, inter_clf =\
        training(inter_feature_data,clf,args.what_to_tune,args.use_stanford_type,args.size_hard_limit)
