"""
Split the data for an instance into 5-fold, compare the performances
when training on same instance or on data from other disaster event
(all data)
"""
...
def load_trained_data(trained_data_dir_name):
    top_features = {}
        
    word_feature_file = os.path.join(trained_data_dir_name,"word_feature")
    cate_feature_file = os.path.join(trained_data_dir_name,"cat_feature")
    classifier_file = os.path.join(trained_data_dir_name,"clf")
    
    word_features = json.load(open(word_feature_file))
    cate_features = json.load(open(cate_feature_file))
    if "USE_STANFORD_TYPE" in cate_features:
        cate_features.remove("USE_STANFORD_TYPE")

    clf = cPickle.load(open(classifier_file))


    return word_features,cate_features,clf
...
def train_the_whole_model(performance_records,max_para,feature_data,\
                         use_stanford_type,clf):
    """ Train the model on the whole sub data and return the features and
    classifier model
    """
    ...
    return top_context_feature,top_category_feature, clf


def show_performance_on_one_instance(instance_test_data,
                                     sub_top_context_feature,
                                     sub_top_category_feature,
                                     use_stanford_type,sub_clf):

    y = []
    record = []
    for single_data in instance_test_data:
        sinlge_record = {
        ...
        if single_data["judgement"] == 1:
            ...
            y.append(0)
        record.append(sinlge_record)

    instance_test_feature_vector =\
        get_sub_feature_vector(instance_test_data,
                               sub_top_context_feature,
                               sub_top_category_feature,
                               use_stanford_type)

    y_pred = sub_clf.predict(instance_test_feature_vector)
...
def training(feature_data,clf,what_to_tune,use_stanford_type,size_hard_limit):
    label,para_set,w_count,c_count = prepare_date(feature_data,what_to_tune,size_hard_limit)
    ...
    for ws_input,cs_input in para_set:
        ...
        for train_index, test_index in splited_data:
            ...
            sub_max_para = tune(train_data,test_data,clf,ws_input,cs_input,use_stanford_type)
    ...
    sub_top_context_feature,sub_top_category_feature, sub_clf =\
        train_the_whole_model(performance_records,max_para,
                             feature_data,use_stanford_type,clf)

    return sub_top_context_feature, sub_top_category_feature, sub_clf


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("trained_data_dir_name")
    ...
    parser.add_argument("--split_data","-sd",type=int,default=0,choices=range(3),
        ...
        """choose to split data on stanford types or not:
                ...
                2: ORGANIZATION only
        """)
    # parser.add_argument("--use_all_data","-ua",action='store_true',
    #     help = 
    #     """When specified, the data of tested event is all used
    #     """
    #     )
    ...
    instance_feature_data = load_data_set_from_single_file(args.feature_data_file,args.split_data)

    label = [x["judgement"] for x in instance_feature_data]

    skf = cross_validation.StratifiedKFold(label,5)

    instance_train_index = []
    instance_test_index = []

    for train_index, test_index in skf:
        instance_train_index = train_index.tolist()
        instance_test_index = test_index.tolist()
        break

    intra_feature_data = []
    for i in train_index:
            intra_feature_data.append(instance_feature_data[ i ])


    # if args.use_all_data:
    #     instance_test_data = instance_feature_data
    # else:
    #     instance_test_data = []
    #     for i in instance_test_index:
    #         instance_test_data.append( instance_feature_data[i] )
    ...
    inter_top_context_feature, inter_top_category_feature, inter_clf =\
        load_trained_data(args.trained_data_dir_name)
    

    show_performance_on_one_instance(instance_test_data,inter_top_context_feature,
                                     inter_top_category_feature,
                                     args.use_stanford_type,inter_clf)
    print "intra:"
    intra_top_context_feature, intra_top_category_feature, intra_clf =\
        training(intra_feature_data,clf,args.what_to_tune,args.use_stanford_type,args.size_hard_limit)
    

    show_performance_on_one_instance(instance_test_data,intra_top_context_feature,
                                     intra_top_category_feature,
                                     args.use_stanford_type,intra_clf)
