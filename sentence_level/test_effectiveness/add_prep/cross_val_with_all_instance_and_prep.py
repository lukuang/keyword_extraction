...
def prepare_date(feature_data,what_to_tune,size_hard_limit):
    ...
    for single_data in feature_data:
        ...
        for w in single_data["word_features"]["verb"]:
            words.add(w)
...
def get_prep_feature(train_data):
    prep_feature = set()
    for single_data in train_data:
        for w in single_data["word_features"]["verb"]:
            prep_feature.add(w)

    return list(prep_feature)
...
def get_sub_feature_vector(sub_feature_date,top_context_feature,
                           top_category_feature,use_stanford_type,prep_feature):
    sub_feature_vector = []
    for single_data in sub_feature_date:
        ...
        for w in top_context_feature:
            if w in single_data["word_features"]["verb"]:
                single_feature_vector.append(single_data["word_features"]["verb"][w])
            else:
                single_feature_vector.append(0)

        for w in prep_feature:
            if w in single_data["word_features"]["prep"]:
                single_feature_vector.append(single_data["word_features"]["prep"][w])
            else:
                single_feature_vector.append(0)
...
def get_feature_vector(train_data,test_data,
                       ...
                       use_stanford_type,prep_feature):
...
def tune(train_data,test_data,clf,ws_input,cs_input,use_stanford_type,what_to_tune):
    ...
    prep_feature = []
    if what_to_tune == 0 or what_to_tune ==2:
        prep_feature = get_prep_feature(train_data)
...
def output_tuning_result(performance_records,max_para,feature_data,\
                         use_stanford_type,clf,dest_dir,what_to_tune):
    ...
    prep_feature = []
    if what_to_tune == 0 or what_to_tune ==2:
        prep_feature = get_prep_feature(feature_data)
    ...
    train_feature_vector, test_feature_vector =\
        get_feature_vector(feature_data,[],
                           top_context_feature,top_category_feature,use_stanford_type,prep_feature)
...
def main():
    ...
    output_tuning_result(performance_records,max_para,
                         feature_data,args.use_stanford_type,clf,args.dest_dir,args.what_to_tune)
