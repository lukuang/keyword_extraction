"""
predict one instance's label using the trained model
from the rest of instances
"""
...
def show_performance_on_one_instance(instance_test_data,
                                     ...
                                     use_stanford_type,sub_clf,dest_file):
    ...
    for i in range(len(y_pred)):
        record[i]["pred"] = y_pred[i]
    print "write to: %s" %(dest_file)
...
def main():
    ...
    for instance in all_data:
        ...
        instance_test_data = all_data[instance]
        ...
        sub_top_context_feature,sub_top_category_feature, sub_clf =\
            train_the_whole_model(performance_records,max_para,
                                 feature_data,args.use_stanford_type,clf)
        
        feature_name = os.path.basename(args.feature_data_dir)
        dest_file_name_list = []
        if args.what_to_tune == 0:
            dest_file_name_list.append(feature_name)
        elif args.what_to_tune == 1:
            dest_file_name_list.append("cat")
        elif args.what_to_tune == 2:
            dest_file_name_list.append(feature_name)
            dest_file_name_list.append("cat")

        if args.use_stanford_type:
            dest_file_name_list.append("us")

        dest_file_name = "_".join(dest_file_name_list)
        dest_file = os.path.join(args.dest_dir,instance,dest_file_name,"record")

        show_performance_on_one_instance(instance_test_data,sub_top_context_feature,
                                         sub_top_category_feature,
                                         args.use_stanford_type,sub_clf,dest_file)
