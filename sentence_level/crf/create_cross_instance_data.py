"""
create data for cross instance 
"""
...
def get_text(text_dir):
    ...
    for single_dir in os.walk(text_dir).next()[1]:
        instance_name = single_dir
        all_text[instance_name]= ""
        ...
        for day_dir in os.walk(single_dir).next()[1]:
            ...
            for single_file in os.walk(day_dir).next()[2]:
                ...
                with  codecs.open(single_file,"r","utf-8") as f:
                    ...
                    all_text[instance_name] += single_text+"\n"
    # print all_text
    return all_text
...
def load_feature_data(feature_dir):
    ...
    for instance in os.walk(feature_dir).next()[2]:
        ...
        feature_data[instance]= reprocess_feature_data(instance_feature_data)
...
def find_non_tagged_sentences(all_text,feature_data):
    ...
    for instance in feature_data:
        non_tagged_sentences[instance] = find_non_tagged_sentences_for_instance(all_text[instance],feature_data[instance])

    return non_tagged_sentences

def find_non_tagged_sentences_for_instance(instance_all_text,instance_feature_data):
    i = 0
    past = []
    for single_data in instance_feature_data:
        sentence = single_data["sentence"]
        index = instance_all_text.find(sentence)
        length  = len(sentence)
        
        if index != -1:

            temp = instance_all_text[:index] +"\n" +instance_all_text[index+length:]
            instance_all_text = temp
            i += 1
        else:
            print instance_all_text
            print past
            print    single_data
    ...
    return sent_tokenize(instance_all_text)
    ...
    for instance in feature_data:
        ...
        for other_instance in feature_data:
            if other_instance != instance:
                ...
                train_non_tagged_sentences += non_tagged_sentences[other_instance]

        test_feature_data = feature_data[instance]
        test_non_tagged_sentences = non_tagged_sentences[instance]
